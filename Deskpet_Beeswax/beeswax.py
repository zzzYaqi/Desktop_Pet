...
from main import storagewindow
...
class MainWindows(QWidget):
    def __init__(self):
        ...
        # 当前宠物
        self.resource='Deskpet_Beeswax\\deskpet'
        ...
        self.skill_begin_index = len(os.listdir(os.path.join(self.resource, 'left', 'skill_begin')))
        self.skill_end_index = len(os.listdir(os.path.join(self.resource, 'left', 'skill_end')))
        self.skill_idle_index = len(os.listdir(os.path.join(self.resource, 'left', 'skill_idle')))
        self.skill_loop_index = len(os.listdir(os.path.join(self.resource, 'left', 'skill_loop')))
        self.special_index = len(os.listdir(os.path.join(self.resource, 'left', 'special')))
        ...
        # 边界
        self.left_bound = -150
        self.right_bound = win32api.GetSystemMetrics(win32con.SM_CXSCREEN) -120
        self.up_bound = -200
    ...
    ###落地及后续动作###
    #假设我们希望落地后的动作是skill begin-idle-loop-end
        if self.position_y!=self.down_bound:
            ...
            self.path = os.path.join(self.resource, self.run_diction, 'skill_begin', str(self.image_index) + '.png')
            self.the_same_image += 1
            self.the_same_image_index_check()
            if self.image_index >= self.skill_begin_index:
                self.skill_begin_flag = False
                self.skill_idle_flag = True
                self.image_index = 0
            if self.the_same_image == 0:
                self.repaint()
        elif self.skill_idle_flag == True:
            self.path = os.path.join(self.resource, self.run_diction, 'skill_idle', str(self.image_index) + '.png')
            self.the_same_image += 1
            self.the_same_image_index_check()
            if self.image_index >= self.skill_idle_index:
                self.skill_idle_flag = False
            ...
            self.path = os.path.join(self.resource, self.run_diction, 'skill_loop', str(self.image_index) + '.png')
            self.the_same_image += 1
            self.the_same_image_index_check()
            if self.image_index >= self.skill_loop_index:
                self.skill_loop_flag = False
            ...
            self.path = os.path.join(self.resource, self.run_diction, 'skill_end', str(self.image_index) + '.png')
            self.the_same_image += 1
            self.the_same_image_index_check()
            if self.image_index >= self.skill_end_index:
                self.skill_end_flag = False
                self.idle_flag = True
                self.image_index = 0
            if self.the_same_image == 0:
            ...
            if self.the_same_image==0:
                self.repaint()
        elif self.run_diction_index!=0 or self.relax_flag==True or self.sit_flag==True or self.sleep_flag==True or self.special_flag == True:
            if self.relax_flag==True:  # relax
                ...
                if self.the_same_image == 0:
                    self.repaint()
            elif self.special_flag==True:  # special
                self.path = os.path.join(self.resource, self.run_diction,'special', str(self.image_index) + '.png')
                self.the_same_image += 1
                self.the_same_image_index_check()
                if self.image_index >= self.special_index:
                    self.special_flag = False
                    self.image_index = 0
                if self.the_same_image == 0:
                    self.repaint()
            ...
            if next_action_index <=50: #relax
                self.relax_flag = True
            elif next_action_index <= 75: #special
    ...
    def menu_init(self):
        ...
        quit_action.setIcon(QIcon('Deskpet_Beeswax\\deskpet\\0.png'))  # 设置右键点开时左边的图片
    ...
    def enterEvent(self, event):  # 鼠标移进时调用
        # print('鼠标移入')
        self.setCursor(
            Qt.ClosedHandCursor)  # 设置鼠标形状。需要from PyQt5.QtGui import QCursor,from PyQt5.QtCore import Qt
        '''
